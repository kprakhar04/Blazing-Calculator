...
const InputMat = ({classes}) => {
  ...
  const [inputMat, setInputMat] = useState(Array.from({ length: rows }, () => Array.from({ length: cols }, () => null)));
  ...
  <div className="dropdown">
      ...
      <TableContainer>
        <Table className="simple table">
          <TableBody>
            {rowsArr.map((e, rowidx) => (
              <TableRow key={rowidx} className={classes.rows}>
                {colsArr.map((ele, colidx) => (
                  <TableCell key={colidx} align="center" className={classes.root}> 
                  <input title='Alphabets Are Not Allowed [ Except i ]' pattern="^[+-i0-9]*$" onChange={e => handleChange(e, rowidx, colidx)} className="inputCell"  name={`${rowidx}''${colidx}`} ref={register({ required: true })} type='text' />
           ...
           <div className = "btnDiv"><button type='submit' className="pulse raise" disabled={errDis.disable}>
      <span style={{color:'black'}}>Compute</span>
          </button>
          <button type="reset" className='pulse raise' id='resetButton'><span style={{color:'black'}}>Reset</span></button></div>
        {errDis.error && (<>
